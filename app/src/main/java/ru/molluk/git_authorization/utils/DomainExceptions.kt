...
sealed class DomainException(override val message: String?, override val cause: Throwable? = null) : IOException(message, cause) {
     ...
     */
    data class Unauthorized(val specificMessage: String? = null, override val cause: Throwable? = null) :
     ...
     */
    data class ApiRateLimitExceeded(val retryAfterSeconds: Int? = null, override val cause: Throwable? = null) :
        DomainException("Превышен лимит запросов к API.${if (retryAfterSeconds != null) " Попробуйте через $retryAfterSeconds сек." else ""}", cause)
    ...
    data class NotFound(val resourceName: String? = null, override val cause: Throwable? = null) :
        DomainException(if (resourceName != null) "Ресурс '$resourceName' не найден." else "Запрошенный ресурс не найден.", cause)
    ...
    data class ServerError(val statusCode: Int, override val cause: Throwable? = null) :
        DomainException("Ошибка на сервере (код: $statusCode). Пожалуйста, попробуйте позже.", cause)

    /**
     * Общая ошибка сети, не классифицированная выше (например, таймауты, проблемы с SSL и т.д.).
     */
    data class GenericNetworkError(val customMessage: String = "Произошла сетевая ошибка. Пожалуйста, попробуйте позже.", override val cause: Throwable? = null):
        DomainException(customMessage, cause)

    /**
     * Неизвестная или непредвиденная ошибка.
     */
    data class Unknown(val customMessage: String = "Произошла неизвестная ошибка.", override val cause: Throwable? = null) :
        DomainException(customMessage, cause)
}